...
export class MainHomeComponent implements OnInit {
  ...
  masonryItems = [];
  posts = [];
  constructor(private routesServices: RutesService, private route: Router) { }
  ...
  /*setAllPost() {
    ...
    this.routesServices.getHomePost().then(res => {
      this.recentPost = res;
      console.log("recentPost->", this.recentPost);
      for (var i = 0; i < this.recentPost.length; i++) {
        let mediaID = this.recentPost[i].featured_media;
